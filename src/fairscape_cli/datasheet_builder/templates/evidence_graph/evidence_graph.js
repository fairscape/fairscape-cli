const evidenceGraphData = window.__EVIDENCE_GRAPH_DATA__;
    ...
    function createEvidenceNode(entityData) {
        ...
        const hasUsedMLModel = !!entityData.usedMLModel && (!Array.isArray(entityData.usedMLModel) || entityData.usedMLModel.length > 0);
        ...
        if ((type === "Dataset" || type === "MLModel") && hasGeneratedBy) { isExpandable = true; }
        if ((type === "Computation" || type === "Experiment") && (hasUsedSoftware || hasUsedDataset || hasUsedSample || hasUsedInstrument || hasUsedMLModel)) { isExpandable = true; }
        ...
        if (isExpandable && type !== "DatasetGroup") {
            ...
            isExpandable = checkRelation('generatedBy') || checkRelation('usedSoftware') || checkRelation('usedDataset') || checkRelation('usedSample') || checkRelation('usedInstrument') || checkRelation('usedMLModel');
        }
    ...
    function expandNodeInternal(node, currentNodes, currentEdges, entityMap) {
        ...
        if ((nodeType === "Dataset" || nodeType === "MLModel") && sourceData.generatedBy) {
        ...
        if ((nodeType === "Computation" || nodeType === "Experiment")) {
             const relations = [
                ...
                { prop: 'usedMLModel', label: 'used model', type: 'uses_model' }
             ];
